...
def step_given_totp_generated(context):
    ...
    context.totp_code, context.expires = TOTPUtils.generate_totp(context.secret, test_config.AUTH)
...
def step_when_totp_verified(context):
    test_config = context.scenario_context.get("test_config")
    context.is_verified = TOTPUtils.verify_totp(context.secret, context.totp_code, test_config.AUTH)


@when("the invalid TOTP code is verified")
def step_when_invalid_totp_verified(context):
    test_config = context.scenario_context.get("test_config")
    context.is_verified = TOTPUtils.verify_totp(context.secret, context.invalid_totp_code, test_config.AUTH)


@when("a secret key is generated")
def step_when_secret_key_generated(context):
    test_config = context.scenario_context.get("test_config")
    context.secret_key = TOTPUtils.generate_secret_key_for_totp(test_config.AUTH)
